...
export default class RegistrationForm {
    ...
    isDifferent(element, handler) {
        if (element.previousValue != element.value) {
            // this keyword remains within the object that called it - not the global ( the call() allows it )
            handler.call(this)
    ...
    emailHandler() {
        ...
        // Creating a timer property on the email object
    ...
    emailAfterDelay() {
        ...
        if (!this.email.errors) {
            axios.post('/doesEmailExist', {email: this.email.value}).then((response) => {
                ...
                if(response.data) {
                    this.email.isUnique = false
                    this.showValidationErrors(this.email, 'That email is already being used')
                }else{
